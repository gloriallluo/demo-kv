...
async fn main() -> Result<(), Box<dyn StdError>> {
    ...
    let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED), opt.listen_addr.port());
    let handle = KVHandle::new(&opt.listen_addr, &opt.snapshot_path, &opt.log_path).await;
    Server::builder()
